...
`;
    ...
    // Try primary model (gemini-2.5-flash), fallback to gemini-1.5-flash or gemini-2.0-flash if needed
    ...
    try {
      ...
      console.warn(`Primary model ${modelName} failed, attempting gemini-1.5-flash:`, primaryErr?.message);
    ...
    const cleanedJson = textResult.replace(/^```json\s*/i, '').replace(/\s*```$/i, '').trim();
